...
<section class="flex items-center justify-center min-h-screen bg-[#7cb1ff]">
    ...
    @endif
        ...
        <form id="resetForm" method="POST" action="{{ route('password.email') }}" class="flex flex-col gap-6">
            @csrf
            <input class="p-3 rounded-xl border border-gray-300 focus:outline-none focus:ring-2 focus:ring-[#002D74]" type="email" name="email" placeholder="Email" required autofocus>
            <button id="resetButton" class="bg-[#002D74] text-white py-3 rounded-xl hover:scale-105 duration-300 hover:bg-[#206ab1] font-medium text-lg">Kirim Link Reset Password</button>
        </form>
